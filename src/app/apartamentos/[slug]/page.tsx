...
export default async function ApartmentDetailPage({ params }: { params: Promise<SlugParams> }) {
  const { slug } = await params
  const apartment = getApartmentBySlug(slug)
    ...
    <div className='min-h-screen bg-background'>
      ...
      <Section>
        <div className='max-w-2xl'>
          <p className='text-foreground text-base md:text-lg leading-relaxed'>{apartment.descripcionLarga}</p>
          {/* Frase de marca */}
          <p className='text-livic-purple italic text-sm mt-4'>"{apartment.frasePosituelo}"</p>
        </div>
      </Section>
      ...
      <Section titulo='Servicios y políticas' acento='yellow'>
        <div className='grid grid-cols-1 sm:grid-cols-2 gap-3'>
          {apartment.servicios.map((svc) => (
            <div key={svc.etiqueta} className='flex items-start gap-3 bg-surface-100 border border-surface-300 rounded-lg p-4'>
              {/* Indicador disponible / no disponible */}
              <div className={`mt-0.5 flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center ${svc.disponible ? 'bg-livic-green/20' : 'bg-surface-300'}`}>
                <span className={`text-xs ${svc.disponible ? 'text-livic-green' : 'text-text-muted'}`}>{svc.disponible ? '✓' : '✕'}</span>
              </div>
              <div>
                <p className={`text-sm font-medium ${svc.disponible ? 'text-foreground' : 'text-text-muted'}`}>{svc.etiqueta}</p>
                {svc.nota && <p className='text-text-muted text-xs mt-0.5'>{svc.nota}</p>}
              </div>
            </div>
          ))}
        </div>
      ...
      <Section titulo='Check-in y Check-out' acento='pink'>
        ...
        {apartment.notas.length > 0 && (
          <div className='mt-5 bg-livic-yellow/10 border border-livic-yellow/30 rounded-xl p-5'>
            <p className='text-livic-yellow text-xs font-semibold uppercase tracking-wide mb-3'>Información importante</p>
      ...
      <Section titulo='Tu anfitrión' acento='pink'>
        <div className='grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-xl'>
          ...
          {apartment.coanfitrion && <HostCard host={apartment.coanfitrion} isPrimary={false} />}
        </div>
      </Section>
      ...
      {apartment.airbnbCalendarUrl && (
        <Section titulo='Disponibilidad' acento='pink'>
          <div className='w-full h-96 overflow-hidden rounded-lg border border-surface-300'>
            <iframe src={apartment.airbnbCalendarUrl} title='Calendario de disponibilidad en Airbnb' className='w-full h-full border-0' allowFullScreen />
          </div>
          <p className='text-text-muted text-sm mt-4 text-center'>Consulta el calendario de disponibilidad en Airbnb</p>
        </Section>
      )}
